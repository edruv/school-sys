<?php
	...
	if (isset($_POST['addsec'])) {
		...
		echo "paso asignacion de horas y fechas".$b;
		/**
		...
		if ($a > 6) {
			...
			*/
			...
			echo "busca secciones".$b;

			if ($busec > 0) {
				echo "entro al if de secciones".$b;
				foreach ($busec as $busc) {
					echo "foreach".$b;
					$diast = explode(',',$busc['dias']);
					$inter = array_intersect($dias,$diast);
					if ($inter) {
						echo "if de inter".$b;
						$idcol = $busc['id'];
						...
						$messa = [
							'type' => 'warning',
							'message' => "la materia de '".$sm['materia'].' ('.$sm['namsec'].")' ya esta utilizando esta aula los dias ".implode(', ',$inter),
							'icon' => 'exclamation'
						];
					}
				}
			}else {
				echo "else de secciones".$b;
				$regsec++;
			}
			...
			$stat = $conn->prepare('SELECT sec.*,us.nombre,us.ap1,us.ap2 from seccion sec
				...
				where sec.maestro=:prof and sec.hra_ini=:hini');
				echo "prepare prof".$b;
...
print_r($buspro);
			if ($buspro >= 1) {
				echo "if profesor".$b;
				foreach ($buspro as $profe) {
					echo "foreach prof".$b;
					$diast = explode(',',$profe['dias']);
					$inter = array_intersect($dias,$diast);
					if ($inter) {
						echo "if inter pro".$b;
						echo $profe['id'].$b;
						$colid = $profe['id'];
						$stat = $conn->prepare('SELECT sec.*,us.nombre,us.ap1,us.ap2 from seccion sec inner join empleado emp on sec.maestro=emp.id inner join user us on emp.id_user=us.id where sec.id=:id');
						$stat->execute(array(':id'=>$colid));
						$bprof = $stat->fetch(PDO::FETCH_ASSOC);
echo "bprof";
print_r($bprof);
						$messa = [
							'type' => 'warning',
							'message' => 'el profesor "'.$bprof['nombre'].' '.$bprof['ap1'].' '.$bprof['ap2'].'" no esta disponible a esta hora '.$hini,
							'icon' => 'exclamation'
						];
					}
				}
