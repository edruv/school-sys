<?php
	...
	if (isset($_POST['ssxcod'])) {
		...
		(!empty($cod)) ? (ctype_digit($cod)) ? $a++ :
			...
			$messa = ['type' => 'danger','icon' => 'exclamation-triangule', 'message' => 'ingresa el codigo de la secretaria'];
		if ($a == 1) {
			...
			$stat->execute(array(':id'=>"$cod",':t'=>'3'));
			$empleado = $stat->fetch(PDO::FETCH_ASSOC);
			if (empty($empleado)) {
				$messa = [
					'type' => 'danger',
					'message' => 'no se ha encontrado a ninguna secretaria con el #'.$cod,
	...
	if (isset($_POST['ssxem'])) {
		...
		if ($a == 1) {
			...
			$stat->execute(array(':mail'=>"$mail",':t'=>'3'));
			$empleado = $stat->fetch(PDO::FETCH_ASSOC);
			if (empty($empleado)) {
				$messa = [
					'type' => 'danger',
					'message' => 'no se ha encontrado a ninguna secretaria con el email'.$mail,
					'icon' => 'exclamation-triangule'
				];
			}else {
				$empleado['link'] = 'secr';
