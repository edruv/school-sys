<?php
	...
	$stat = $conn->prepare("SELECT sec.id,ma.nombre materia, sec.namsec, sec.cupo, DATE_FORMAT(sec.hra_ini,'%H:%i') hra_ini, DATE_FORMAT(sec.hra_fin,'%H:%i') hra_fin, sec.dias, ed.nombre edif, sec.aula, DATE_FORMAT(sec.f_ini,'%d/%m/%y') f_ini, DATE_FORMAT(sec.f_fin,'%d/%m/%y') f_fin, ca.carrera, us.nombre, us.ap1, us.ap2, emp.id cod from seccion sec
	...
	echo $twig->render('seccs_view.twig',compact('title','visec'));
